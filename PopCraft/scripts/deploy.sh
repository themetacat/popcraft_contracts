...
if [ $INIT != false ]; then
    ...
    echo $SYSTEM_ADDRESS
    # cast send $WORLD_ADDRESS --priority-gas-price 0.0001gwei --rpc-url $RPC_URL --private-key $PRIVATE_KEY "update_app_system(address, string)" $SYSTEM_ADDRESS $APP_NAME
    # cast send $WORLD_ADDRESS --gas-price 0.002gwei --priority-gas-price 0.0001gwei --rpc-url $RPC_URL --private-key $PRIVATE_KEY "update_app_system(address, string)" $SYSTEM_ADDRESS $APP_NAME
